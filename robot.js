...
export function makeMetalRobot() {
  ...
  triangleGeometry.setAttribute('position',
    ...
    body.children.forEach((child)=>{//=>:一回しか使わない時に便利
...
head.add(eyeR);
head.children.forEach((child)=>{//=>:一回しか使わない時に便利
  child.castShadow = true;
  child.reciveShadow = true;
});
...
metalRobot.add(head);
  // 影についての設定
metalRobot.children.forEach((child)=>{//=>:一回しか使わない時に便利
  child.castShadow = true;
  child.reciveShadow = true;
});
  // 作成結果を戻す
...
export function makeCBRobot() {
  ...
  legR.position.set(-legSep/2, legLen/2,legSep/2);
  cardboardRobot.add(legR);
  const legL = new THREE.Mesh(legGeometry, cardboardMaterial);
  legL.position.set(legSep/2, legLen/2,legSep/2);
  ...
  // 影についての設定
  cardboardRobot.children.forEach((child)=>{//=>:一回しか使わない時に便利
    child.castShadow = true;
    child.reciveShadow = true;
  });
  // 再生結果を戻す
